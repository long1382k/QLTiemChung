...
namespace DXApplication2
{
     public partial class FormTK_ThanhTichBS : UserControl
     {
          ...
          private void btnBaoCaoTTBS_Click(object sender, EventArgs e)
          {
               KetNoi kn = new KetNoi();
               DataTable dt = kn.LoadDataBCTT("XemThanhTich1BS", "@MaBS", ma, dateFromBS.Text, dateToBS.Text);
               if (dt.Rows.Count == 0)
               {
                    XtraMessageBox.Show("Không có dữ liệu để xuất");
               }
               ...
               {
                    XtraReportTTBS reportTTBS = new XtraReportTTBS();
                    reportTTBS.DataSource = dt;
                    reportTTBS.ShowPreviewDialog();
               }
          }

          private void dataTKTTBS_CellClick(object sender, DataGridViewCellEventArgs e)
          {
               int index = dataTKTTBS.CurrentRow.Index;
               ma = dataTKTTBS.Rows[index].Cells[1].Value.ToString();
          }
     }
}