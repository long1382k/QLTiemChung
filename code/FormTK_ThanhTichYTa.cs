     ...
     public partial class FormTK_ThanhTichYTa : UserControl
     {
          ...
          private void btnBieuDoYTa_Click(object sender, EventArgs e)
          {
               try
               {
                    ...
                    chartControlTLYTa.DataSource = kn.LoadDataTK("TiLeThanhTichYTa", dateFromYTa.Text, dateToYTa.Text);
                    chartControlTLYTa.Show();
          ...
          private void btnBCTTYTa_Click(object sender, EventArgs e)
          {
               KetNoi kn = new KetNoi();
               DataTable dt = kn.LoadDataBCTT("XemThanhTich1YTa", "@MaYTa", ma, dateFromYTa.Text, dateToYTa.Text);
               if (dt.Rows.Count == 0)
               {
                    XtraMessageBox.Show("Không có dữ liệu để xuất");
               }
               ...
               {
                    XtraReportTTYTa reportTTYTa = new XtraReportTTYTa();
                    reportTTYTa.DataSource = dt;
                    reportTTYTa.ShowPreviewDialog();
               }
          }

          private void dataTKTTYTa_CellClick(object sender, DataGridViewCellEventArgs e)
          {
               int index = dataTKTTYTa.CurrentRow.Index;
               ma = dataTKTTYTa.Rows[index].Cells[1].Value.ToString();
          }
     }
}
