...
namespace DXApplication2
{
     class KetNoi
     {
          ...
          // load du lieu cho bao cao thanh tich bac si, nhan vien, y ta
          public DataTable LoadDataBCTT(string proc, string param, string ma, string dateFrom, string dateTo)
          {
               DataTable table = new DataTable();
               SqlCommand command = new SqlCommand();
               command.Connection = connection;
               ...
               command.Parameters.AddWithValue("@dayFrom", SqlDbType.Date).Value = dateFrom;
               command.Parameters.AddWithValue("@dayTo", SqlDbType.Date).Value = dateTo;
               SqlDataAdapter adapter = new SqlDataAdapter(command);
               adapter.Fill(table);
               connection.Close();
               return table;
